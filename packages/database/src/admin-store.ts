...
export class SqliteAdminStore {
  ...
  public updateSetting(input: {
    ...
    const run = this.database.transaction(() => {
      ...
      this.database
        ...
        .run(ulid(), input.key, valueJson, input.actorUserId, BigInt(this.now()));
      this.recordAudit({
        actorUserId: input.actorUserId,
  ...
  public health(): {
    ...
    const balanceTotals = this.database
      .prepare(
        `SELECT
           ...
           COALESCE(SUM(CASE WHEN a.account_type = 'user' THEN ab.amount ELSE 0 END), 0)
             AS users,
    ...
    const relief = this.database
      ...
      .get() as { amount: bigint };
    const cutoff = BigInt(this.now() - 30 * 24 * 60 * 60 * 1_000);
  ...
  private probeDatabaseReadWrite(): boolean {
    const nonce = ulid();
    try {
      const probe = this.database.transaction(() => {
        this.database
          .prepare(
