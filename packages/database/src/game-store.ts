...
export class SqliteGameStore {
  ...
  public createHorse(input: HorseWrite): HorseRecord {
    ...
    this.database
      .prepare(
        `INSERT INTO horses
         (id, name, status, running_style, speed, start, acceleration, stamina, late_kick,
    ...
    this.database
      .prepare('UPDATE horses SET coat_color = ? WHERE id = ?')
      .run(input.coatColor ?? 'chestnut', id);
  ...
  public openBettingPools(input: {
    ...
    const run = this.database.transaction(() => {
      const race = this.getRace(input.raceId);
  ...
  public purchaseBet(input: PurchaseBetInput): PurchasedBet {
    const run = this.database.transaction((): PurchasedBet => {
      const duplicate = this.database
        .prepare('SELECT id FROM bets WHERE idempotency_key = ?')
  ...
  public planSeedLiquidity(raceId: string): {
    ...
    const winTotals = recentTotals('win');
    const trifectaTotals = recentTotals('trifecta');
    const financial = parseRaceFinancialSettings(race.simulationConfigJson, race.kind);
    const plan = planAdaptiveSeedLiquidity(race.kind, winTotals, trifectaTotals, {
...
function parseRaceFinancialSettings(
  ...
  readonly raceBetLimit: number;
  readonly seedLiquidityClamp: SeedClamp;
} {
