...
export const SIMULATION_VERSION = 'official-20hz-v3-preference-axis';
export const SIMULATION_TICK_SECONDS = 0.05;
export const TIMELINE_SAMPLE_TICKS = 2;
...
export function simulateOutcomeOnly(input: SimulationInput, seed: string): readonly number[] {
  return runSimulation(input, new Xoshiro128StarStar(seed), false).finishOrder.map(
...
export function simulateManyOutcomes(
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    outcomes.push(simulateOutcomeOnly(input, trialSeed));
  }
  return outcomes;
}
